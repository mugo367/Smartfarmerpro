...
public class employeetypeReportActivity extends AppCompatActivity {
    ...
    List<Addemployee> addemployeetypeList;

    public static File EmtypeFile;
    private File emtypefile;
    private PDFView pdfView;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_report_employeetype);

        Toolbar toolbar = findViewById(R.id.tooltype);
        ...
        getSupportActionBar().setTitle("Employee Type Report");

        type = findViewById(R.id.spinnertype);
        generate = findViewById(R.id.generatetype);
        pdfView = findViewById(R.id.typeemp_pdf_viewer);
        ...
        addemployeetypeList = new ArrayList<>();

        //create file
        emtypefile = new File("/storage/emulated/0/Report/");
        //check if they exist, if not create them(directory)
            if ( !emtypefile.exists()) {
            emtypefile.mkdirs();	        }
        EmtypeFile = new File(emtypefile, "employeetype.pdf");
        //fetch payment and disabled users details;
        generate.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                previewemptypeReport();
                fetchemptype();
            }
        });
    ...
    private void fetchemptype() {
        String typeemp = type.getSelectedItem().toString();
        employeeRef.orderByChild("emptype").equalTo(typeemp).addValueEventListener(new ValueEventListener() {
            @Override
       ...
       You were adding an object of type PaymentUsers yet the arraylist expects obejct of type DisabledUsers
             */ addemployeetypeList.clear();
                        addemployeetypeList.add(addemployee);

                    }

                }	                //create a pdf file and catch exception beacause file may not be created
                try {
                    createEmployeetypeReport(addemployeetypeList);
    ...
    private void createEmployeetypeReport(List<Addemployee> addemployeeList) throws DocumentException, FileNotFoundException {
        ...
        FileOutputStream output = new FileOutputStream(EmtypeFile);
        ...
        Chunk typeText = new Chunk("Date of Employment", white);
        ...
        for (int i = 0; i < addemployeeList.size() ; i++){
            Addemployee addemployee = addemployeeList.get(i);
        ...
        String des = type.getSelectedItem().toString();
        Calendar calendar = Calendar.getInstance();
        SimpleDateFormat dateFormat = new SimpleDateFormat("MM/dd/yyyy");
        String date = dateFormat.format(calendar.getTime());
        document.add(new Paragraph(" REPORT ON EMPLOYEE TYPE ("+des+") " +"AS AT "+date+"\n\n", g));
    ...
    public void previewemptypeReport()
    {
