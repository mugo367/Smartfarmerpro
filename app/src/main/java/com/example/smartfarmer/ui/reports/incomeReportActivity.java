...
public class incomeReportActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Toolbar toolbar = findViewById(R.id.toolincome);
        ...
        pdfView = findViewById(R.id.income_pdf_viewer);
        ...
        addincomeList = new ArrayList<>();

        //create file
        incomefile = new File("/storage/emulated/0/Report/");
        //check if they exist, if not create them(directory)
        if ( !incomefile.exists()) {
            incomefile.mkdirs();	        }
        pFile = new File(incomefile, "income.pdf");
    ...
    private void fetchincome() {
        incomeRef.orderByChild("transactiontype").equalTo("Income").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                    ...
                    if(addincome1.getId().equals(firebaseUser.getUid())) {
                        ...
                        addincome.setTransactiondate(snapshot.child("transactiondate").getValue().toString());
                        addincome.setTransactionname(snapshot.child("transactionname").getValue().toString());
                        addincome.setTransactioncost(snapshot.child("transactioncost").getValue().toString());
                        addincome.setTransactiondetails(snapshot.child("transactiondetails").getValue().toString());
       ...
       You were adding an object of type PaymentUsers yet the arraylist expects obejct of type DisabledUsers
             */
                ...
                try {
                    createIncomeReport(addincomeList);
    ...
    private void createIncomeReport(List<Addincome> addincomeList) throws DocumentException, FileNotFoundException {
        ...
        for (int i = 0; i < addincomeList.size() ; i++){
            Addincome addincome = addincomeList.get(i);

            String id = String.valueOf(i +1);
            String Date = addincome.getTransactiondate();
            String Name= addincome.getTransactionname();
            String Cost = addincome.getTransactioncost();
            String details = addincome.getTransactiondetails();
    ...
    public void previewincomeReport()
    {
